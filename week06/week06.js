...
function validateAge(theAge, theClass)
{
	...
	if(theAge >= 0 && theAge <= 118 && theAge.length != 0 && ageTest.test(theAge) == true)
		{
...
// Second problem: SSN 
function validateSSN(theSSN, theClass)
{
	var SSNTest = /^\s*\d{3}-\d{2}-\d{4}\s*$/;
	var SSNTest3 = /[A-Za-z]/;

	if(SSNTest.test(theSSN) == true && SSNTest3.test(theSSN) == false && theSSN.length != 0)
		{
		document.getElementsByClassName(theClass)[0].style.visibility = 'hidden';
		//console.log("3");
		}
...
{
	                //1-12          1-31                     	1753-2100                        
	var dateTest = /^\s*([1-9]|1[0-2])\/([1-9]|[12][0-9]|3[01])\/(175[3-9]|17[6-9][0-9]|1[89][0-9]{2}|20[0-9]{2}|2100)\s*$/;
...
{             
	...
	var state45 = /^\s*\T\V\s*$/; var state46 = /^\s*\V\A\s*$/; var state47 = /^\s*\W\A\s*$/; var state48 = /^\s*\W\V\s*$/;
	 ...
	 if ( (state1.test(theId) == true || state2.test(theId) == true || state3.test(theId) == true || 
	   	   ...
	   	   state49.test(theId) == true || state50.test(theId) == true) && stateTest1.test(theId) == false)
		{
		document.getElementsByClassName(theClass)[0].style.visibility = 'hidden';
		console.log("10");
		}
